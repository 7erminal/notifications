...
type Notifications struct {
	...
	Status              *Notification_status `orm:"rel(fk)"`
	ServiceId           *Services            `orm:"rel(fk)"`
	NotificationFor     *Users               `orm:"rel(fk)"`
...
func GetNotificationsById(id int64) (v *Notifications, err error) {
	...
	if err = o.QueryTable(new(Notifications)).Filter("Id", id).RelatedSel().One(v); err == nil {
		return v, nil
	}
	return nil, err
}
...
func DeleteNotifications(id int64) (err error) {
	...
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&Notifications{Id: id}); err == nil {
