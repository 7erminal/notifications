...
type Items struct {
	...
	AvailableColors string      `orm:"column(available_colors);size(250);null"`
	AvailableSizes  string      `orm:"column(available_sizes);size(250);null"`
	CategoryId      *Categories `orm:"column(category_id);rel(fk)"`
	Country         *Countries  `orm:"column(country);rel(fk)"`
	...
	Id              int         `orm:"column(item_id);auto"`
	ItemName        string      `orm:"column(item_name);size(80)"`
	ItemPriceId     *ItemPrices `orm:"column(item_price_id);rel(fk)"`
	Material        string      `orm:"column(material);size(400);null"`
	ModifiedBy      int         `orm:"column(modified_by);null"`
	Quantity        int         `orm:"column(quantity);null"`
	Weight          string      `orm:"column(weight);size(20);null"`
}
